...
Original file is located at
    https://colab.research.google.com/drive/1Ky9Aeqr6NUxMg_VPha9Jojbe5qnnZdYP
"""
...
print(scaled_data)                       #Menampilkan array hasil normalisas
...
# Asumsikan prediksi 10 hari ke depan telah dihitung dan disimpan dalam predictions_10_days
predictions_10_days = predictions_10_days.flatten()  # Flatten array hasil prediksi

# Tentukan tanggal awal (tanggal terakhir dari dataset asli)
start_date = df.index[-1]  # Ambil tanggal terakhir dari dataset asli
start_date = pd.to_datetime(start_date)  # Pastikan dalam format datetime

# Membuat daftar tanggal untuk 10 hari ke depan
date = [start_date + timedelta(days=i) for i in range(1, 11)]

# Membuat variabel prediksi
Prediksi = predictions_10_days

# Menampilkan hasil dalam bentuk DataFrame untuk kejelasan
result_df = pd.DataFrame({
    'Date': [d.strftime('%Y-%m-%d') for d in date],  # Format tanggal menjadi string
    'Prediksi': price  # Prediksi harga
})

# Menampilkan DataFrame
print(result_df)

# Asumsikan data harga aktual dan prediksi telah dihitung
# Harga aktual (dari dataset asli)
actual_prices = df['Price']  # Semua data harga aktual
...
# Membuat daftar tanggal untuk prediksi
date_predictions = [start_date + timedelta(days=i) for i in range(1, 11)]

# Grafik harga aktual dan prediksi
plt.figure(figsize=(14, 7))

# Plot harga aktual
plt.plot(actual_prices, label='Harga Aktual', color='blue')  # Harga aktual dari dataset

# Plot prediksi
plt.plot(date_predictions, predictions_10_days, label='Prediksi 10 Hari', color='orange', linestyle='--',)

# Tambahkan detail pada grafik
plt.title('Grafik Harga Aktual dan Prediksi', fontsize=20)
